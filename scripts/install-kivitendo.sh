...
# getestet auf Debian 13 im LXC Container priviligiert

# System-Varibale
IP=$(ip addr show eth0 | grep -o 'inet [0-9]\+\.[0-9]\+\.[0-9]\+\.[0-9]\+' | grep -o [0-9].*)

# de_DE.UTF-8 aktivieren
# 1. Locale in /etc/locale.gen aktivieren (falls noch nicht geschehen)
sed -i 's/# de_DE.UTF-8 UTF-8/de_DE.UTF-8 UTF-8/' /etc/locale.gen

# 2. Locales generieren
locale-gen

# 3. Systemweite Default-Locale setzen (optional, aber empfohlen)
echo "LANG=de_DE.UTF-8" > /etc/default/locale

# 4. Für die aktuelle Shell sofort wirksam machen (optional)
...
tee kivitendo.conf >/dev/null <<EOF
[authentication]
...
[secrets]
# Passphase used to encrypt/decrypt secrets stored in the kivitendo client database.
# While any length is allowed, it will get stretched into a 256bit AES key - more is better.
master_key =
...
cd /etc/postgresql/17/clustername
sed -i '/^[[:space:]]*#listen_addresses/s/^([[:space:]]*)\(#\)/\1/' postgresql.conf


mv /etc/postgresql/17/clustername/pg_hba.conf /etc/postgresql/17/clustername/pg_hba.conf.original
sleep 5

tee /etc/postgresql/17/clustername/pg_hba.conf  >/dev/null <<EOF
...
mv /etc/postgresql/17/clustername/postgresql.conf /etc/postgresql/17/clustername/postgresql.conf.original
sleep 5

su - postgres -c "psql -d template1 -c 'CREATE EXTENSION IF NOT EXISTS plpgsql;'"
sleep 5

# Passwort fuer Superuser postgres gesetzt
su - postgres -c "psql -c \"ALTER USER postgres WITH PASSWORD 'kivitendo';\""

sleep 5
a2enmod fcgid

tee /etc/apache2/sites-available/kivitendo.apache2.conf  >/dev/null <<EOF
AddHandler fcgid-script .fcgi   # oder .fpl, je nach Dispatcher
...
<DirectoryMatch "/var/www/kivitendo-erp/(config|\.git)/">
    Require all denied
</DirectoryMatch>
EOF
